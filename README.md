...
- **Papan ringkas + panel info 3 kolom**: papan 7x12 (7 kotak per baris, 84 kotak total, semua baris tetap tampil tanpa scroll) mengisi bagian atas layar dengan kotak yang lebih besar (rasio papan lebih landai dari sebelumnya supaya lebih mendekati lebar layar penuh), dengan panel 3 kolom di bawahnya (masing-masing ~sepertiga lebar): kolom kiri **log langkah** (mencatat setiap lemparan dadu & perpindahan setiap pemain, bukan cuma kejadian spesial — naik tangga, kena ular, dapat kartu, dst., terbaru di atas, tiap baris diwarnai sesuai warna pemain yang bersangkutan dengan jarak ekstra saat giliran berganti pemain), kolom tengah **dadu & kartu power-up** (dadu diperkecil agar proporsional dengan kolom), kolom kanan **daftar pemain** (lingkaran inisial karakter + nama + posisi, giliran aktif disorot). Kotak terakhir (finish) diberi pola kotak-kotak hitam putih ala bendera balap. Tangga & ular didesain dengan jarak antar kotak yang lebih pendek (maksimal ~3 baris) supaya papan tidak terlihat berantakan, digambar dengan dua rel + anak tangga (bukan garis polos) untuk tangga dan badan menyambung meruncing ke ekor dengan kepala bermata sipit & lidah bercabang untuk ular, plus backdrop vektor bertema di belakang papan. Pion pemain menampilkan emoji karakter yang dipilih pemain di layar setup (bukan cuma bulatan polos, ukurannya juga lebih besar), dengan label inisial kecil di bawah pion untuk memudahkan identifikasi, dan bergerak selangkah demi selangkah dengan tempo yang lebih santai (bukan lompat cepat) sambil berbunyi "langkah" tiap pindah kotak. Dadu putih dengan titik (pip) seperti dadu asli, berubah abu-abu otomatis saat bukan giliran pemain tersebut, dengan efek suara "kocokan" (beberapa tick cepat berturut-turut, ala bel sepeda) selama animasi putar. Naik tangga dan kena ular kini punya efek suara yang jelas berbeda (nada positif vs nada negatif). Tombol kembali & suara jadi ikon kecil melayang transparan di pojok atas, info giliran tampil sebagai popup singkat (bukan bar permanen) setiap kali giliran berganti, dan kartu power-up di kolom tengah membungkus rapi ke baris baru bila tidak cukup lebar (tidak lagi meluber).
...
- **Layar setup ringkas**: semua pilihan (mode permainan, jumlah pemain, karakter per pemain) memakai dropdown, bukan baris tombol/slider, supaya layar setup lebih rapi dan singkat. Latar layar setup putih bersih (tanpa backdrop bertema), supaya elemen berwarna (dropdown, tombol, chip karakter) lebih menonjol. Tidak ada lagi pemilihan tema visual — layar permainan tetap memakai satu tema visual (Hutan Tropis) secara konsisten.
...
- Personalisasi pemain: nama & karakter bisa diedit per pemain di layar setup, dengan karakter unik antar pemain.
